    ...
    describe(`Adapter: ${adapterName}`, () => {
      ...
      describe('checkout', () => {
        ...
        const query = `mutation m($token: String!){ checkout(token: $token) {
            ...
            label
            total
            items { name user { id } description price quantity image { id } }
            user { id }
            charge
          } }`;
        test(
        ...
        test(
          ...
          runner(setupKeystone, async ({ context }: { context: KeystoneContext }) => {
            ...
            expect(result2.data!.checkout.items[0].name).toEqual(product2.name);
            expect(result2.data!.checkout.items[0].user.id).toEqual(user2.id);
            expect(result2.data!.checkout.items[0].description).toEqual(product2.description);
            expect(result2.data!.checkout.items[0].price).toEqual(product2.price);
            expect(result2.data!.checkout.items[0].quantity).toEqual(3);
            expect(result2.data!.checkout.items[0].image.id).toEqual(product2.image.id);
            expect(result2.data!.checkout.items[1].name).toEqual(product1.name);
            expect(result2.data!.checkout.items[1].user.id).toEqual(user2.id);
            expect(result2.data!.checkout.items[1].description).toEqual(product1.description);
            expect(result2.data!.checkout.items[1].price).toEqual(product1.price);
            expect(result2.data!.checkout.items[1].quantity).toEqual(2);
            expect(result2.data!.checkout.items[1].image.id).toEqual(product1.image.id);
          })
        );
      });
      ...
      describe('addToCart', () => {
        ...
        test(
          ...
          runner(setupKeystone, async ({ context }: { context: KeystoneContext }) => {
            ...
            const { data, errors } = await _context.graphql.raw({
              query,
              variables: { productId: '123456781234567812345678' },
            });
        ...
        test(
          ...
          runner(setupKeystone, async ({ context }: { context: KeystoneContext }) => {
            const _context = asUser(context, '123456781234567812345678');
            const { data, errors } = await _context.graphql.raw({
              query,
              variables: { productId: '123456781234567812345678' },
            });
